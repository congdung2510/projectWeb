 ...
 * @author ADMIN
 */
@WebServlet(name = "ListPCServlet", urlPatterns = {"/pc"})
public class ListPCServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao = new DAO();
        int endPage = dao.getToltalPC() / 9 + (dao.getToltalPC() % 9 == 0 ? 0 : 1);
        String pageNumber = request.getParameter("page");
        if (pageNumber == null) {
            pageNumber = "1";
        }
        List<PC> list = dao.pagingPC(Integer.parseInt(pageNumber));
